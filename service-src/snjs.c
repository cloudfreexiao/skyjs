...
init_cb(struct snjs *l, struct skynet_context *ctx, const char * args, size_t sz) {
	...
	char *loader_code = read_file(loader);
	if (loader_code != NULL) {
		JSValue lret = JS_Eval(l->jsc, loader_code, strlen(loader_code), loader, JS_EVAL_TYPE_GLOBAL);
	...
	const char *sock_loader = optstring(ctx, "js_socket", "./js/socket.js");
	char *sock_code = read_file(sock_loader);
	if (sock_code != NULL) {
		JSValue sret = JS_Eval(l->jsc, sock_code, strlen(sock_code), sock_loader, JS_EVAL_TYPE_GLOBAL);
		skynet_free(sock_code);
		if (JS_IsException(sret)) {
			dump_exception(l, "snjs socket loader error");
			return 1;
		}
		JS_FreeValue(l->jsc, sret);
	}
	const char *cluster_loader = optstring(ctx, "js_cluster", "./js/cluster.js");
	char *cl_code = read_file(cluster_loader);
	if (cl_code != NULL) {
		JSValue cret = JS_Eval(l->jsc, cl_code, strlen(cl_code), cluster_loader, JS_EVAL_TYPE_GLOBAL);
		skynet_free(cl_code);
		if (JS_IsException(cret)) {
			dump_exception(l, "snjs cluster loader error");
			return 1;
		}
		JS_FreeValue(l->jsc, cret);
	}
...
snjs_release(struct snjs *l) {
	...
	JS_FreeValue(l->jsc, l->new_map_fn);
