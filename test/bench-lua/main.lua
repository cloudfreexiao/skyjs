-- run-bench.js phase-1 driver (stock Lua side). Mirrors
-- test/service/bench-suite-main.js case-for-case; see that file for the output
...
-- bench-suite-main.js can stay at file scope)
local c_echo, l_echo
...
local function case_send(n)
    ...
    -- the FIFO drain barrier, mirroring bench-suite-main.js
    mark("send_self")
    local t0 = skynet.hpc()
...
local function case_startup(name, launch)
    ...
    -- first message (snlua loads lazily), mirroring bench-suite-main.js
    mark(name)
    local t0 = skynet.hpc()
    local handles = {}
...
skynet.start(function()
    ...
    -- warmup (untimed), mirroring bench-suite-main.js
