...
skynet.start(() => {
    skynet.dispatch("text", async (msg) => {
        ...
        ok = check(Array.isArray(t1) && t1[0] === 10 && t1[1] === 20 && t1[2] === 30, "array table") && ok;
        // Map-compat accessors on array-only table (luaarray wrapper)
        ok = check(typeof t1.get === "function", "luaarray .get exists") && ok;
        ok = check(t1.get(1) === 10 && t1.get(2) === 20 && t1.get(3) === 30, "luaarray .get 1-based") && ok;
        ok = check(t1.has(1) === true && t1.has(3) === true && t1.has(0) === false && t1.has(4) === false, "luaarray .has") && ok;
        ok = check(t1.size === 3 && t1.size === t1.length, "luaarray .size") && ok;
        // .get/.has/.size must be non-enumerable (no pollution)
        ok = check(JSON.stringify(t1) === "[10,20,30]", "luaarray JSON.stringify clean") && ok;
        ...
        ok = check(rb[0] === "seri_b_ok" && rb[1] === 43 && Array.isArray(rb[2]) &&
            rb[2][0] === 1 && rb[2][1] === 2, "lua protocol roundtrip") && ok;
