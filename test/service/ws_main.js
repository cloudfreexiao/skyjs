...
skynet.timeout(1, async () => {
    ...
    try {
        ...
        // ---- Binary echo ----
        {
            ...
            const bin_hex = ab_hex(bin);
            websocket.write(id, bin, "binary");
            const r = await websocket.read(id);
            check("binary_echo", !r.close && r.type === "binary" &&
                ab_hex(r.data) === bin_hex,
                "type=" + r.type + " hex=" + ab_hex(r.data) + " want=" + bin_hex);
            websocket.close(id, 1000);
        }
        ...
        // ---- Client close handshake ----
        {
            ...
            check("client_close", websocket.is_close(id), "not closed");
        }
        ...
        // ---- Handshake rejection (invalid HTTP request) ----
        {
            ...
            try {
                ...
                try {
                    ...
                    rej_ok = line.includes("400") || line.includes("Bad") ||
                        line.includes("HTTP");
                } catch (e) {
            ...
            check("handshake_reject", rej_ok, "no rejection detected");
        }
    ...
    if (fail_count === 0) {
        ...
        console.log("WS FAIL total=" + fail_count);
    }
});
