...
async function run_tests() {
    ...
    const reentry_tokens = ["left", "right"];
    const reentry_results = await Promise.all(reentry_tokens.map(token =>
        skynet.call(chain_h, "text", "reentry:" + token)));
    for (let i = 0; i < reentry_tokens.length; i++) {
        const expected = "A(B1(C(D(B2(" + reentry_tokens[i] + ")))))";
        if (reentry_results[i] !== expected) {
            throw new Error("async reentry mismatch: expected " + expected +
                ", got " + reentry_results[i]);
        }
    }

    const dual_result = await skynet.call(chain_h, "text", "dual:pair");
    const expected_dual = "A(BDUAL(X(pair-first),X(pair-second)))";
    if (dual_result !== expected_dual) {
        throw new Error("async dual mismatch: expected " + expected_dual + ", got " + dual_result);
    }
    ...
    const result = "R1=" + r1 + "|R2=" + r2 + "|REENTRY=" + reentry_results.join(",") +
        "|DUAL=" + dual_result + "|CONC=" + rs.join(",") + "|ERR=" + err_caught;
