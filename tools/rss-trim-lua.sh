#!/bin/zsh
# usage: rss-trim-lua.sh <case> — dedicated lua-node RSS peak for one bench case
# (bootstrap runs test/bench-lua/main-trim.lua with the case as snlua param)
cd "$(dirname "$0")/.."
CASE=$1
LOG=build/trim-lua-log-$CASE.txt
cat > build/config-trim-lua <<EOF
thread = 4
harbor = 0
logservice = "logger"
profile = true
bootstrap = "snlua bootstrap"
start = "test/bench-lua/main_trim $CASE"
...
EOF
(cd 3rd/skynet && ./skynet ../../build/config-trim-lua) > "$LOG" 2>&1 &
PID=$!
PEAK=0
N=0
